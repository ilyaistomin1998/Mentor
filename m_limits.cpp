...
char *lim3411t (const range& r, char *text)
{
    ...
    chprintf(text, "lim(x->0, (1+cos(%s))/(1-cos(%s)))=inf", polynomial(buf[0],1,a,"x"),polynomial(buf[1],1,b,"x"));
    return text;
}
...
char *lim451t (const range& r, char *text)
{
    ...
    sprintf(text, "lim(x->+inf, !((%s)/(%s))^((%s)/(x+1)))=%s",
        polynomial(buf[0], 3, a1,"x^2",b1,"x",c1,""),
        polynomial(buf[1], 2, a2,"x^2",b2,"x",c2,""),
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * */
/*
char *lim03 (const range& r, char *text)
{
    char buf[10][100];
    int a1,a2,b,c,d;
    do{
...
char *lim20 (const range& r, char *text)
{
    char buf[10][BUFSZ];
    int a,b;
    do{
...
char *lim21 (const range& r, char *text)
{
    char buf[10][BUFSZ];
    int a,b;
    do{
...
char *lim22 (const range& r, char *text)
{
    char buf[10][BUFSZ];
    int a;
    do{
...
char *lim23 (const range& r, char *text)
{
    char buf[10][BUFSZ];
    int a,b,c,p;
    do{
 ...
 * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
 /*
char *lim24 (const range& r, char *text)
{
    char buf[10][BUFSZ];
    int a,b,c,p;
    do{
...
char *limddd (const range& r, char *text)
{
    char buf[10][BUFSZ];
    int a1,b1,c1, a2,b2,c2, d;
    do{
...
/*
char *lim901 (int&a1,int&b1,int&c1,int&a2,int&b2,int&c2,int&d, const range& r, char *text)
{
    char buf[10][BUFSZ];
    do{
        a1=rndr(r), b1=rndr(r), c1=rndr(r);
